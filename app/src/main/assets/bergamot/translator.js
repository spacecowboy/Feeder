 ...
 */
 export class TranslatorBacking {
    ...
    constructor(options) {
         ...
         * @type {(error: Error)}
         */
        this.onerror = this.options.onerror || (err => { throw err; });
    }
    ...
    async loadWorker() {
        ...
        // Helper to load a file via XHR (fetch doesn't support file:// on Android WebView)
        const loadText = (url) => new Promise((resolve, reject) => {
            ...
            xhr.onerror = () => reject(new Error(`Failed to load ${url}: network error`));
            xhr.send();
        });

        // Load both scripts via XHR
        const workerScriptUrl = new URL('./worker/translator-worker.js', import.meta.url);
        const emscriptenUrl = new URL('./worker/bergamot-translator-worker.js', import.meta.url);
        const [workerScript, emscriptenScript] = await Promise.all([
            loadText(workerScriptUrl.href),
            loadText(emscriptenUrl.href)
        ]);

        // Replace importScripts call with inline Emscripten script content.
        // Patch Emscripten's `var Module` declaration to use self.Module so it
        // sees our configured instantiateWasm callback instead of creating a
        // blank shadow Module via var hoisting.
        // Stict mode inside class methods makes `this` undefined. Patch to use `self`.
        const finalEmscripten = emscriptenScript
            .replace(
            ...
            .replace(
                ...
                'var global_object = self;'
            );
        const combinedScript = workerScript.replace(
            "self.importScripts('bergamot-translator-worker.js')",
            finalEmscripten
        );

        // Create Worker from Blob URL to avoid file:// Worker restrictions on Android WebView
        ...
        const wasmBinary = await new Promise((resolve, reject) => {
            ...
            xhr.onload = () => {
                if (xhr.status === 200 || xhr.status === 0) {
                    ...
                    reject(new Error(`Failed to fetch WASM: status=${xhr.status}`));
                }
            };
            xhr.onerror = () => reject(new Error(`Failed to fetch WASM: network error`));
            xhr.send();
        });
        workerOptions.wasmBinary = wasmBinary;
        ...
        setTimeout(async () => {
            ...
            if (!worker && this.workers.length < this.workerLimit) {
                ...
                try {
                    ...
                    this.workers = this.workers.filter(w => w !== placeholder);
                    const error = new Error(`Could not initialise translation worker: ${e.message}`);
                    this.onerror(error);
