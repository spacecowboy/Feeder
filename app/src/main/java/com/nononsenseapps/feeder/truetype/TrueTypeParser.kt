    ...
    return inputStream.use { stream ->
        ...
        if (fvarOffset > 0 && fvarLength > 0) {
            ...
            for (axisIndex in 0.toUInt() until axisCount) {
                ...
                val minValue = fixedType32ToFloat(bytes.toUByteArray())
                ...
                val defaultValue = fixedType32ToFloat(bytes.toUByteArray())
                ...
                val maxValue = fixedType32ToFloat(bytes.toUByteArray())
...
@OptIn(ExperimentalUnsignedTypes::class)
fun fixedType32ToFloat(bytes: UByteArray): Float {
    var integer: UInt = 0.toUInt()
    for (i in 0 until 2) {
        integer = (integer shl 8) or bytes[i].toUInt()
    }
    ...
    // Convert to float
    val floatValue = integer.toFloat() + (decimal.toFloat() / 65536.0f)
